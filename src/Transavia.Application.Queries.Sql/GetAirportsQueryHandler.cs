    ...
    public class GetAirportsQueryHandler : IGetAirportsQueryHandler
    {
        ...
        public async Task<GetAirportsResult> Handle(GetAirportsQuery request, CancellationToken cancellationToken)
        {
            using (var connection = _connectionFactory.Create())
            {
                ...
                var airports = await connection.QueryAsync<Airport>(getAirportsQuery, new { request.SkipCount, request.TakeCount});
                var totalBooksCount = await connection.ExecuteScalarAsync<int>(getAirportsTotalCountQuery);
